...
backup_dir = codex / "ai-router-workspace" / "backup" / "sessions" / f"manual-single-restore-019dd8b9-swf-{now}"
...
def codex_running():
    try:
        import subprocess
        output = subprocess.check_output(["powershell", "-NoProfile", "-Command", "Get-Process | Where-Object { $_.ProcessName -match '^Codex$|^codex$' } | Select-Object -First 1 -ExpandProperty Id"], text=True, stderr=subprocess.DEVNULL)
