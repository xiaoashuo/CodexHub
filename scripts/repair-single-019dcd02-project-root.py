...
target_id = "019dcd02-b72c-7a22-9b65-7a3432156a56"
project = r"D:\idea\source\company\snif\source\trunk\national-custom-admin\national-admin-vue3"
project_lower = project[:1].lower() + project[1:]
project_sqlite = r"\\?\D:\idea\source\company\snif\source\trunk\national-custom-admin\national-admin-vue3"
provider = "aimai1"
now = int(time.time())
...
backup_dir = codex / "ai-router-workspace" / "backup" / "sessions" / f"manual-single-restore-019dcd02-root-{now}"
...
con.execute(
    "update threads set cwd=?, model_provider=?, archived=0, updated_at=?, updated_at_ms=? where id=?",
    (project_sqlite, provider, now, now * 1000, target_id),
)
con.execute(
...
prompt = row["first_user_message"] or row["title"] or target_id
